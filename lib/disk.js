    ...
    case 'darwin':
        module.exports = function(statEmitter, options) {
            return new Promise((resolve, reject) => {
                const df = "df -Pkl | grep -v Capacity | awk '{ print $1\" \"$2\" \"$3\" \"$4\" \"$5\" \"$6\" \"$7}'";
                exec(df, {stdio:['inherit','pipe','pipe']}, function(err, output, code) {
                    if (err) statEmitter.emit('error', err);
                    ...
                    drives.forEach(function(drive) {
                        ...
                        if (!options.threshold || options.threshold === 0 || usedpct > options.threshold) {
                            statEmitter.emit('disk', { filesystem: filesystem, mount: mount, usedpct: usedpct, total: total * 1024, free: free * 1024 });
                        }
                    });
